...
namespace lumos {
ImageData4u8 readPng(const std::filesystem::path &input) {
  ...
  if (!data_handle) {
    throw runtime_error("failed to read png: {}", input);
  }
  ImageData4u8 res(ImageDataView4u8(
...
void savePng(const std::filesystem::path &output, const ImageData4u8 &pic) {
  ...
  if (!success) {
    throw runtime_error("failed to save png: {}", output);
  }
}
